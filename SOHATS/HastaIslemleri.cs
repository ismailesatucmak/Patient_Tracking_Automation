...
namespace SOHATS
{
    ...
    {
        public HastaIslemleri(AnaForm anaForm, FormControl formControl)
        {
            InitializeComponent();
        ...
        private void GetTahlil()
        {
            ...
            if (string.IsNullOrEmpty(cbOncekiIslemler.Text))
            {
                MessageBox.Show("Lütfen bir tarih seçiniz");
                return;
            }
            ...
            DateTime tarih;
            if (!DateTime.TryParse(cbOncekiIslemler.Text, out tarih))
            {
            ...
            foreach (sevk sevk in sevkler)
            {
                ...
                int birimFiyat;
                if (int.TryParse(sevk.birimfiyat, out birimFiyat))
                {
                    miktar += birimFiyat;
                }
            }
            ...
            if(sevkler.Count != 0)
            {
                if (!string.IsNullOrEmpty(sevkler[0].taburcu) && sevkler[0].taburcu.ToUpper() == "TRUE")
                {
        ...
        private void printDocument1_PrintPage(object sender, System.Drawing.Printing.PrintPageEventArgs e)
        {
            ...
            {
                var row = dgwTahlilveİslemler.Rows[i];
            ...
            // Doktor bilgisi için null kontrolü
            if (dgwTahlilveİslemler.Rows.Count > 0 && dgwTahlilveİslemler.Rows[0].Cells[4].Value != null)
            {
                int drKod;
                if (int.TryParse(dgwTahlilveİslemler.Rows[0].Cells[4].Value.ToString(), out drKod))
                {
                    kullanici doktor = databaseControl.DoktorAdi(drKod);
                    if (doktor != null)
                    {
        ...
        {
            if(string.IsNullOrEmpty(cbPoliklinik.Text))            
                return;
            
            sevk sevk = databaseControl.Sira(cbPoliklinik.Text, dtpSevkTarihi.Value);

            if(sevk == null || string.IsNullOrEmpty(sevk.sira))
            {
                txtSiraNo.Text = "1";
            }
            else
            {
                int sira;
                if (int.TryParse(sevk.sira, out sira))
                {
        ...
        {
            if (string.IsNullOrEmpty(cbYapilanİslem.Text))
            {
            ...
            txtBirimFiyat.Text = islem?.birimFiyat ?? "0";
        }
        ...
        private void btnTaburcu_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                if(sevkler != null && sevkler.Count != 0 && !string.IsNullOrEmpty(sevkler[0].taburcu) && sevkler[0].taburcu.ToUpper() == "TRUE")
                {
