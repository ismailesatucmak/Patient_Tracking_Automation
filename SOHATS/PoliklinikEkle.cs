...
namespace SOHATS
{
    ...
    {
        public PoliklinikEkle(AnaForm anaForm,FormControl formControl)
        {
            InitializeComponent();
        ...
        private void cbPoliklinikAdi_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter )
            {
                ...
                if(p.poliklinikadi == null)
                {
                    ...
                    formControl.Open(new PoliklinikSayfasi(poli.id, poli.poliklinikadi, Convert.ToBoolean(poli.durum), poli.aciklama));
                }
                else
                {
                    formControl.Open(new PoliklinikSayfasi(p.id, p.poliklinikadi, Convert.ToBoolean(p.durum), p.aciklama));
                }
            }
        }
    }
